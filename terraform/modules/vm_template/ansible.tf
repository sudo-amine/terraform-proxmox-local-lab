...
resource "null_resource" "run_ansible" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      echo "${var.vm_template.name} ansible_host=${var.vm_template.ip} ansible_user=${var.vm_user} ansible_ssh_private_key_file=${var.vm_ssh.private_key_path} ansible_become=true" >> $inventory_file
...
data "http" "vm_template_status" {
  ...
  url        = "https://${var.proxmox.host}:8006/api2/json/nodes/${var.proxmox.node}/qemu/${var.vm_template.id}/status/current"
  request_headers = {
    Authorization = "PVEAPIToken=${var.proxmox.api_user}!${var.proxmox.api_token_id}=${var.secret_proxmox_api_token}"
  }
}